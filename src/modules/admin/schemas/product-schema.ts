...
export const ProductSchema = z
  .object({
    ...
    orderLimit: z
      ...
      .refine(
        (orderLimit) =>
          !isNaN(parseInt(orderLimit)) && parseInt(orderLimit) > 0,
        {
          message:
            'El límite de compra debe ser un número entero positivo mayor a 0',
        },
      ),
    img: z.any(),
  })
  .strict()
  .refine((data) => parseFloat(data.price) >= parseFloat(data.discount), {
